"""Тесты порядка FSM регистрации: номер → компания → тип → дата."""
...
async def test_start_registration_asks_company_first(
    ...
    assert await fsm_state.get_data() == _base_data()
...
async def test_type_selected_goes_to_date(test_db, make_message, fsm_state):
    await fsm_state.set_state(RegisterContainer.waiting_for_type)
    await fsm_state.set_data(
        {**_base_data(), "company_id": 1, "company_name": "Ромашка"}
    )
    msg = make_message("40HQ")
    await register.type_selected(msg, fsm_state)

    assert (
        await fsm_state.get_state()
        == RegisterContainer.waiting_for_arrival_date.state
    )
    assert (await fsm_state.get_data())["container_type"] == "40HQ"
    assert "Когда контейнер прибыл" in msg.answer.call_args[0][0]


async def test_type_skip_goes_to_date(test_db, make_message, fsm_state):
    await fsm_state.set_state(RegisterContainer.waiting_for_type)
    await fsm_state.set_data(
        {**_base_data(), "company_id": 1, "company_name": "Ромашка"}
    )
    msg = make_message()
    await register.type_skip(msg, fsm_state)

    assert (
        await fsm_state.get_state()
        == RegisterContainer.waiting_for_arrival_date.state
    )
    assert (await fsm_state.get_data())["container_type"] is None


async def test_arrival_today_finalizes(test_db, make_message, fsm_state):
    ...
    assert container["arrival_date"] is not None
    assert await fsm_state.get_state() == ContainerSection.card.state


async def test_arrival_transit_finalizes(test_db, make_message, fsm_state):
    company_id = await db_comp.add_company(name="Ромашка")
    await fsm_state.set_state(RegisterContainer.waiting_for_arrival_date)
    await fsm_state.set_data(
        {
    ...
    assert container["arrival_date"] is None
    assert await fsm_state.get_state() == ContainerSection.card.state


async def test_manual_date_finalizes(test_db, make_message, fsm_state):
    company_id = await db_comp.add_company(name="Ромашка")
    ...
    assert container is not None
    assert container["status"] == "on_terminal"
    assert container["arrival_date"] == "2026-06-01 00:00:00"
    assert await fsm_state.get_state() == ContainerSection.card.state


async def test_full_flow_creates_container(test_db, make_message, fsm_state):
    """Сквозной happy-path: компания → тип → дата → запись в БД."""
    await register.start_registration(
    ...
    await register.type_selected(make_message("40HQ"), fsm_state)
    await register.arrival_today(make_message(), fsm_state, role="full")
    ...
    # откуда принимается номер следующего контейнера.
    assert await fsm_state.get_state() == ContainerSection.card.state
...
async def test_finalize_incomplete_data_resets(
    ...
    """Сессия без company_id (пережила деплой) не падает с KeyError."""
    await fsm_state.set_state(RegisterContainer.waiting_for_arrival_date)
    await fsm_state.set_data(_base_data())
    msg = make_message()
    await register.arrival_today(msg, fsm_state, role="full")
