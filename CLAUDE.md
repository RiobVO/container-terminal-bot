...
Клиент проверил на тестовом боте и подтвердил («ВСЁ ЧЕТКО»). Смержено в `main`. Осталось:
1. Деплой: `ssh deploy@188.166.127.192`, `cd ~/container && git pull && docker compose up -d --build`, проверить `docker compose logs -f bot`.
2. Чек-лист после деплоя — п. 5.3 из `docs/superpowers/plans/2026-06-04-registration-flow-redesign.md` (порядок шагов там описан до финального уточнения клиента — фактический: компания → тип → дата).
3. Подчистить: удалить этот раздел и `test_local.db`; владелец аннулирует тестовый токен в BotFather.
