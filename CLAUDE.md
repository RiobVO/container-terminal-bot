...
## Незакрытая задача: деплой нового порядка регистрации (актуально на 2026-06-04)

Клиент утвердил порядок: **номер → компания → тип → дата** («Сначала компанию. Потом тип. Потом время»). Реализовано на ветке `feat/registration-flow-redesign` (коммит `0fcdca7`, 75 тестов зелёные), **не смержено и не задеплоено**. Также на ветке: ввод номера следующего контейнера прямо из карточки (`card_fallback`), отбраковка номеров как имён компаний, мягкий сброс осиротевших Redis-сессий. Имена FSM-состояний не менялись — живые сессии переживут деплой (застрявшие на старых шагах завершатся мягко, худший случай — тип «не указан», правится в карточке).

Осталось:
1. Владелец проверяет флоу на `@testkont_bot` (локальный запуск: тестовый BOT_TOKEN из BotFather, `ADMIN_IDS=7082498953,1234177510`, `DATABASE_PATH=test_local.db`; токен не хранится, выдаёт владелец).
2. Мерж ветки в `main`, обычный деплой (`git pull && docker compose up -d --build`).
3. Чек-лист после деплоя — п. 5.3 из `docs/superpowers/plans/2026-06-04-registration-flow-redesign.md` (порядок шагов там описан до финального уточнения клиента — сверяться с фактическим: компания → тип → дата).
4. Подчистить: удалить этот раздел, `test_local.db`, ветку после мержа; владелец аннулирует тестовый токен.
