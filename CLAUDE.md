...
- **Перед деплоем проверить ветку на сервере:** `sudo -u deploy git -C /home/deploy/container status -sb` — должна быть `main` и без уникальных коммитов (`git log origin/main..HEAD` пуст). Прецедент 2026-06-05: сервер стоял на ветке `claude/*` от удалённой сессии — `git pull` отвечал «up to date», docker пересобирал старый код, а на самой ветке жили боевые коммиты, которых не было в `main` (пришлось мержить, не переключать вслепую).
- **Git на сервере — только от имени `deploy`** (`sudo -u deploy git -C /home/deploy/container ...`). Под root падает с «dubious ownership»; добавлять safe.directory для root нельзя — pull под root перепишет владельца файлов и сломает деплои.
