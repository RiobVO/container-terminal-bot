...
## Незакрытая задача: порядок шагов регистрации (актуально на 2026-06-04)

Ветка `feat/registration-flow-redesign` (6 коммитов, 74 теста зелёные) готова, **не смержена и не задеплоена**. Блокер: клиент дал противоречивый ответ («Нет наоборот») на вопрос о порядке шагов. Ему задан уточняющий вопрос («1 — сначала компания, как сейчас; 2 — сначала дата») и дан доступ к тестовому боту `@testkont_bot` (поднимается локально: тестовый BOT_TOKEN из BotFather, `ADMIN_IDS=7082498953,1234177510`, `DATABASE_PATH=test_local.db`; токен не хранится, выдаёт владелец).

Сделано на ветке:
- Порядок регистрации: номер → дата → компания → тип (в проде: номер → компания → дата → тип). Имена FSM-состояний не менялись — Redis-сессии переживают деплой.
- Ввод номера следующего контейнера прямо из карточки (`card_fallback`) — клиент это **не оспаривал**, нужно при любом исходе.
- Защиты: отбраковка номеров контейнеров как имён компаний; мягкий сброс осиротевших Redis-сессий в `process_company`/`_finalize`.

Как добить по ответу клиента:
- **«2» (дата первой)** → мерж ветки в main, обычный деплой, прогнать чек-лист п. 5.3 из `docs/superpowers/plans/2026-06-04-registration-flow-redesign.md`.
- **«1» (компания первой, как в проде)** → на той же ветке откатить только перестановку шагов: вернуть старые переходы (`start_registration` → компания, `process_company` → дата, дата-хендлеры → тип), убрать проверку `status` из входного guard'а `process_company` (в старом порядке `status` на шаге компании легитимно отсутствует). Оставить: `card_fallback`, отбраковку номеров-имён, guard полноты в `_finalize`, тесты (адаптировать под порядок).
- Спека: `docs/superpowers/specs/2026-06-04-registration-flow-redesign-design.md` (не в git — `docs/superpowers` в `.gitignore`).
- После закрытия удалить этот раздел и файл `test_local.db`, владелец аннулирует тестовый токен.
